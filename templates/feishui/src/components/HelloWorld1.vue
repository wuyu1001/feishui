    ...
    <el-row class="elrow">
      <el-col :span="24"
        ...
        <el-table :data="student.table_zzjg_xsxx" border style="width: 100%">
          ...
          <el-table-column
            v-for="(v, i) in student.columns.filter(item => item.field !='is_changed')"
            :prop="v.field"
            :label="v.title"
            :width="v.width"
            :key="i"
...
export default {
  ...
  methods: {
    ...
    handleSizeChange1(val) {
      this.size1 = val;
      this.getTableData("zzjg_xsxx", val, this.currentPage1);
    },
    handleCurrentChange1(val) {
      this.getTableData("zzjg_xsxx", this.size1, val);
    },
    pwdChange(row, index, is_save) {
      ...
      if (is_save) {
        this.$axios
          .post(
            "http://127.0.0.1:5000/modifyTable/zzjg_xsxx",
    ...
    search() {
      ...
      this.getTableData("zzjg_xsxx", this.size1, this.currentPage1, v_where);
    },
    reset() {
      this.student.columns.forEach((x) => {
        x.search_value = "";
      });
      this.getTableData("zzjg_xsxx", this.size1, this.currentPage1, undefined);
    },
  },
  mounted() {
    this.getTableData("zzjg_xsxx", this.size1, 1);
  },
};
</script>
